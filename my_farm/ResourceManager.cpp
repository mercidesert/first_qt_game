...
void ResourceManager::loadAll() {
    ...
    // 猫咪图片加载
    for(int i=0; i<4; i++) {
        catDown[i] = lKeep(QString(":/images/cat_down%1.png").arg(i+1), 32, 32);
        catUp[i]   = lKeep(QString(":/images/cat_up%1.png").arg(i+1), 32, 32);
        catLeft[i] = lKeep(QString(":/images/cat_left%1.png").arg(i+1), 32, 32);
        catRight[i]= lKeep(QString(":/images/cat_right%1.png").arg(i+1), 32, 32);
        catLie[i]  = lKeep(QString(":/images/cat_lie%1.png").arg(i+1), 32, 32);
        catLick[i] = lKeep(QString(":/images/cat_lick%1.png").arg(i+1), 32, 32);
        catSit[i]  = lKeep(QString(":/images/cat_sit%1.png").arg(i+1), 32, 32);
    }
    for(int i=0; i<2; i++) {
        catCurl[i] = lKeep(QString(":/images/cat_curl%1.png").arg(i+1), 32, 32);
        catJump[i] = lKeep(QString(":/images/cat_jump%1.png").arg(i+1), 32, 32);
    }
}
